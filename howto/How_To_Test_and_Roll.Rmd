...
[Drexel Business Analytics Meetup](https://www.meetup.com/analyticslebow/)  
[Drexel MS Business Analtyics](https://www.lebow.drexel.edu/land/masters/business-analytics)  
[Drexel MS Marketing](https://www.lebow.drexel.edu/land/masters/marketing)  
...
[R Markdown file that created slides](https://raw.githubusercontent.com/eleafeit/testandroll/master/howto/How_To_Test_and_Roll.Rmd)

Those familiar with R Markdown can open the file in R Studio and run the code as we go along.   
...
You do not need to know how plan and analyze an A/B test using hypothesis testing, but see [previous R Ladies Philly workshop on A/B testing](https://www.youtube.com/watch?v=QXpYtM-Zlxg&t=4s).
...
- the opportunity cost of the test (You are send the wrong treatment to half your customers!)  
...
But, when the priors for A and B are the same, then the posterior is is just the average profit in the test. 
```{r}
post_mean_A <- mean(test_data[test_data$group=="A", "profit"])
post_mean_B <- mean(test_data[test_data$group=="B", "profit"])
```
